...
\begin{sectionbox}
	...
	Konstruktiv: $\Delta \varphi = 2\pi n$\\
	Destruktiv: $\Delta \varphi = (2n+1)\pi$\\
	\textbf{Allgemein:}
	\begin{itemize}
		\item Gangunterschied $\Delta = a\sin \alpha = \frac{x}{n}\sin \alpha=\frac{\varphi}{2\pi}\lambda$
		\item Phasendifferenz $\varphi = 2\pi \frac{\Delta}{\lambda}=2\pi a \frac{\sin \alpha}{n\lambda}$
		\item Winkel Max: $\sin \alpha_n = \pm n   \frac{\lambda}{a}$
		\item Winkel Min: $\sin \alpha_n = \pm (n+\frac{1}{2})  \frac{\lambda}{a}$
	\end{itemize}
	\subsubsection{Doppelspalt}
	Int max: $\Delta = \lambda n$ (Konstruktiv)\\
	Int min: $\Delta = (2n+1)\frac{\lambda}{2}$ (Destruktiv)\\
	$\Delta = a \sin \alpha' \approx a \frac{x}{d}$\\
	Kleinwinkelnäherung: $\frac{x}{d}\approx \tan \alpha \approx \sin \alpha \approx \alpha \approx \alpha'$\\
	$\Rightarrow \frac{\Delta}{a} = \frac{x}{d}$
